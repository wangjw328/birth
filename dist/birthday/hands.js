...
export class HandControls{
 constructor(video,onFrame,onStatus){Object.assign(this,{video,onFrame,onStatus,active:false,loading:false,stream:null,detector:null,generation:0,lastTime:-1,lastRun:0,raf:0,stable:{name:'none',since:0},mobile:matchMedia('(pointer: coarse)').matches});}
 async prepare(){
  ...
  if(!this.preparing)this.preparing=(async()=>{
   ...
   const options={baseOptions:{modelAssetPath:new URL('./vendor/hand_landmarker.task',import.meta.url).href,delegate:'GPU'},runningMode:'VIDEO',numHands:this.mobile?1:2,minHandDetectionConfidence:.5,minHandPresenceConfidence:.5,minTrackingConfidence:.5};
   try{this.detector=await HandLandmarker.createFromOptions(vision,options);}catch{options.baseOptions.delegate='CPU';this.detector=await HandLandmarker.createFromOptions(vision,options);}
 ...
 async start(){
  ...
  try{
   ...
   const pending=navigator.mediaDevices.getUserMedia({video:{width:{ideal:this.mobile?480:640},height:{ideal:this.mobile?360:480},frameRate:{ideal:24,max:30},facingMode:'user'},audio:false});
   ...
   this.onStatus('3 / 3 · 手势追踪已开启。摄像头画面不会显示，也不会上传。');this.loop();
 ...
 loop(){
  ...
  if(document.hidden||now-this.lastRun<(this.mobile?95:65)||this.video.readyState<2||this.video.currentTime===this.lastTime)return;
 ...
 drawLandmarks(hands){const c=document.getElementById('hand-landmarks');if(!c)return;const ctx=c.getContext('2d');ctx.clearRect(0,0,c.width,c.height);const links=[[0,1,2,3,4],[0,5,6,7,8],[5,9,10,11,12],[9,13,14,15,16],[13,17,18,19,20],[0,17]];ctx.strokeStyle='rgba(255,255,255,.94)';ctx.lineWidth=2.2;ctx.lineCap='round';ctx.lineJoin='round';ctx.shadowColor='rgba(142,218,198,.7)';ctx.shadowBlur=5;for(const hand of hands){for(const chain of links){ctx.beginPath();chain.forEach((i,j)=>{const p=hand[i];ctx[j?'lineTo':'moveTo']((1-p.x)*c.width,p.y*c.height);});ctx.stroke();}ctx.shadowBlur=3;ctx.fillStyle='rgba(255,255,255,.96)';for(const p of hand){ctx.beginPath();ctx.arc((1-p.x)*c.width,p.y*c.height,2.5,0,Math.PI*2);ctx.fill();}}}
