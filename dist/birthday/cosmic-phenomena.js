...
export class CosmicPhenomena{
 constructor(scene){
  ...
  this.wormhole=new T.Mesh(new T.PlaneGeometry(12,12),new T.ShaderMaterial({transparent:true,depthTest:false,depthWrite:false,side:T.DoubleSide,uniforms:{time:{value:0},accent:{value:new T.Color('#b5b8c8')}},vertexShader:`varying vec2 pos;void main(){pos=uv*2.-1.;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}`,fragmentShader:`varying vec2 pos;uniform float time;uniform vec3 accent;void main(){vec2 p=pos;float r=length(p),a=atan(p.y,p.x);if(r>.96)discard;float spiral=.5+.5*sin(a*3.-r*34.+time*.23);float thin=exp(-pow((r-.44)*52.,2.));float disk=exp(-pow((r-.57)*13.,2.))*(.2+spiral*.65);float halo=exp(-pow((r-.48)*7.,2.))*.12;float core=1.-smoothstep(.35,.405,r);vec3 tint=mix(vec3(.26,.31,.40),accent,.4);vec3 color=mix(tint*(thin*.9+disk*.5+halo),vec3(.003,.005,.009),core);float alpha=max(core,clamp(thin*.85+disk+halo,0.,.9))*(1.-smoothstep(.86,.96,r));gl_FragColor=vec4(color,alpha);
#include <colorspace_fragment>
}` }));this.wormhole.position.set(-19,10,-32);this.wormhole.rotation.set(.28,-.12,.4);this.wormhole.renderOrder=20;this.group.add(this.wormhole);
